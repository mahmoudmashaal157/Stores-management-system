...
import com.example.mashaal_stores.databinding.FragmentInsertStore5Binding
import kotlinx.coroutines.launch
import java.time.LocalDate
import java.time.format.DateTimeFormatter

class insert_Store5 : Fragment() {
    private val viewmodel : Store1_viewmodel by activityViewModels()
    val c=0
    val positiveButtonClick = { dialog: DialogInterface, which: Int -> }
    private lateinit var binding : FragmentInsertStore5Binding
    ...
    binding = FragmentInsertStore5Binding.inflate(inflater,container,false)
                    ...
                    if(num < sadr){
                        ...
                        try{
                            lifecycleScope.launch {
                                viewmodel.insert_store5(item1(0, name, type, curdate, num, sadr, num - sadr))
                                nameInsert.setText("")
                                numInsert.setText("")
                                sadrInsert.setText("")
                            }
