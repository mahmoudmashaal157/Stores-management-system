...
import com.example.mashaal_stores.databinding.ActivityUpdateActStore5Binding
...
class Update_act_Store5 : AppCompatActivity() {
    private lateinit var binding : ActivityUpdateActStore5Binding
    var sadr = 0.0
    var ward = 0.0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityUpdateActStore5Binding.inflate(layoutInflater)
        setspinneradapter()
        val pth :String = Store3_Database.getdatabase(this).openHelper.writableDatabase.path.toString()
        Log.v("pth",pth)

        lifecycleScope.launch {
             ss = intent.getStringExtra("name").toString()
            Log.v("name1", ss)
            val db = Store5_Database.getdatabase(applicationContext)
            try{
                ...
                updateButton.setOnClickListener {
                    ...
                    if (wardUpdate.text.isNotEmpty() && sadrUpdate1.text.isEmpty()) {
                        ...
                        if (sadrUpdate1.text.toString().toDouble() > num) {
                            val builder = AlertDialog.Builder(this@Update_act_Store5)
                        ...
                        if (sadrUpdate1.text.toString()
                                .toDouble() > num + wardUpdate.text.toString().toDouble()
                        ) {
                            val builder = AlertDialog.Builder(this@Update_act_Store5)
                            builder.setTitle("خطأ حسابي ")
                    ...
                    if (update_flag == 1) {
                        try {
                            val item =
                                item1(item1.uid, name, type, curdate, ward_update, sadr_update, num)
                            viewmodels.update_store5(item)
                            ss= name
                            flag = 1
                        } catch (E: Exception) {
                    ...
                    if (flag == 1) {
                        Toast.makeText(applicationContext, "تم التعديل بنجاح  ", Toast.LENGTH_SHORT)
                            .show()
                    }
                    update_flag = 1
                }
